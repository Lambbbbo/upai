...
def run_module():
    ...
    module_args = dict(
        ...
        typ=dict(type='str', required=False),
    ...
    if typ == 'core':
        core_md5 = version.split('-')[2]
        core_version = '.'.join(version.split('-')[0].split('.')[0:-1]) + '.0'
        tar = '{}-core-{}-{}.tar.gz'.format(project, core_version, core_md5)
    else:
        app_version = version.split('-')[0]
        app_md5 = version.split('-')[-1]
        if 'core' in version:
            tar = '{}-app-{}-{}.tar.gz'.format(project, app_version, app_md5)
        else:
            tar = '{}-{}-{}.tar.gz'.format(project, app_version, app_md5)
    ...
    if cdn:
        ...
        if hostname.startswith("MIX"):
            ...
            server_ip = ['127.0.0.1:83', addr, '183.131.24.37:80']
